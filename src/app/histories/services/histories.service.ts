...
export class HistoriesService {
  Histories: Observable<Array<{ type: string; name: string; time: string; status: string }>>
  private _histories: BehaviorSubject<
    Array<{ type: string; name: string; time: string; status: string }>
  > = new BehaviorSubject<Array<{ type: string; name: string; time: string; status: string }>>([])
  ...
  private handleUpload(arg: any, status: string) {
    const to = arg.parents.join('/') + '/' + arg.filename
