...
describe('TreeViewComponent', () => {
  ...
  it('should display tree correctly', fakeAsync(() => {
    ...
    expect(
      fixture.nativeElement
        .querySelectorAll('.tree-container > app-tree-item')[0]
        .querySelectorAll('.sub-items > app-tree-item').length,
    ).toBe(3)
    expect(
      fixture.nativeElement
        .querySelectorAll('.tree-container > app-tree-item')[0]
        .querySelectorAll('.sub-items > app-tree-item')[0]
        .querySelectorAll('.sub-items > app-tree-item').length,
    ).toBe(1)
  }))
  ...
  it('should add items to correct node on ItemsEnumerated', fakeAsync(() => {
    ...
    expect(
      fixture.nativeElement
        .querySelectorAll('.tree-container > app-tree-item')[0]
        .querySelectorAll('.sub-items > app-tree-item')[2]
        .querySelectorAll('.sub-items > app-tree-item').length,
    ).toBe(1)
    expect(
      fixture.nativeElement
        ...
        .querySelectorAll('.sub-items > app-tree-item')[2]
        .querySelectorAll('.sub-items > app-tree-item')[0]
        .querySelectorAll('.tree-icon i')[1].classList,
    ).toContain('ion-ios-archive')
  }))
  ...
  it('should set item enumerated to false on ItemsEnumerated', fakeAsync(() => {
    component.rootNodes = [
      {
        name: 'root',
        subItems: [{ name: 'child1' }, { name: 'child2' }],
      },
    ]
    s3Svc.ItemsEnumerated.emit({
      parents: ['root'],
