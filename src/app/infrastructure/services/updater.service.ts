...
export class UpdaterService {
  ...
  private _updaterStatus: BehaviorSubject<UpdaterStatus> = new BehaviorSubject<UpdaterStatus>(
    UpdaterStatus.NoUpdateAvailable,
  )

  private _updateVersion: BehaviorSubject<string> = new BehaviorSubject<string>('')
  constructor(private electron: ElectronService, private noti: NotificationsService) {
  ...
  init() {
    this.electron.onCD('Updater', (event, arg) => {
      if (arg.msg === 'update-available') {
        ...
        const notification = this.noti.info(
          'Update Available',
          'Click here to install update, the app will restart automatically to update',
          {
            timeOut: 5000,
          },
        )
        ...
        setTimeout(() => {
          this.electron.send('Updater', 'commence-install-update')
        }, 7 * 1000)
      }
    })
