...
@Component({
  ...
  styleUrls: ['../page.scss', './settings-page.component.scss'],
})
...
export class SettingsPageComponent extends SubscriptionComponent implements OnInit {
  ...
  ngOnInit() {
    this.recordSubscription(
      this.requestTracking.RequestCounts.subscribe(_ => {
        this.requestCounts = _
      }),
    )
    this.recordSubscription(
      this.s3.DownloadPath.subscribe(d => {
        this.downloadPath = d
      }),
    )
    this.recordSubscription(
      this.updater.updaterStatus.subscribe(_ => {
        this.updaterStatus = _
      }),
    )
    this.recordSubscription(
      this.updater.updateVersion.subscribe(_ => {
        this.updateVersion = _
      }),
    )
    this.recordSubscription(
      this.updater.updateDownloadProgress.subscribe(_ => {
        this.updateDownloadProgress = _
      }),
    )
    this.recordSubscription(
      this.analytics.enabled.subscribe(_ => {
